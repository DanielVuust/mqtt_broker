...
pub fn read_package_length(buffer: &[u8], mut index: usize) -> (usize, usize){
    //Plus 2 as the length number is not included the package identifyer package(index 0) and the length bytes (index 1-4). this should be changed when adding support to packges more than 255.
    let length = buffer[index] + 2;
    index += 1;
    (length as usize, index)
}

pub fn read_uft_8_string_with_length_bytes(buffer: &[u8], mut index: usize) -> (String, usize){
...
pub fn read_uft_8_string_with_end_index(buffer: &[u8], mut current_index: usize, end_index: usize) -> (String, usize){
